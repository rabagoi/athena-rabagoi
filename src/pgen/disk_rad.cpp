...
Real Omega0;
static Real alpha;
static Real T0, H0, t0, mol_weight, heatrate, opacity, rho0_rad;
} // namespace
...
void Mesh::InitUserMeshData(ParameterInput *pin) {
  ...
  rho0_rad = pin->GetReal("radiation", "density_unit");
      ...
      for (int j=js; j<=je; ++j) {
        ...
        for (int i=is; i<=ie; ++i) {
          ...
          for (int n=0; n<pnrrad->n_fre_ang; ++n) {
              ...
              pnrrad->ir(k,j,i,n) = pow(PoverR(x1*sinth, x3, x1*costh), 4.0);
          }
        }
      }
    }
  ...
  for (int k=ks; k<=ke; ++k) {
    ...
    for (int j=js; j<=je; ++j) {
      ...
      for (int i=is; i<ie; ++i) {
        ...
        phdif->nu(HydroDiffusion::DiffProcess::iso,k,j,i) = alpha*PoverR(Rcyl, th, phi)*sqrt(R*R*R/1.0);
    ...
    for (int j=1; j<=ngh; ++j) {
      for (int i=is; i<=ie; ++i) {
        for (int ifr=0; ifr<nfreq; ++ifr) {
          for(int n=0; n<nang; ++n){
            int ang = ifr*nang + n;
            Real& miux=prad->mu(1,k,js-j,i,n);
            if (miux < 0.0)
              ir(k,js-j,i,ang) = ir(k,js,i,n);
            else
              ir(k,js-j,i,ang) = 0.0;
          }
        }
    ...
    for (int j=1; j<=ngh; ++j) {
      for (int i=is; i<=ie; ++i) {
        for (int ifr=0; ifr<nfreq; ++ifr) {
          for(int n=0; n<nang; ++n){
            int ang = ifr*nang + n;
            Real& miux=prad->mu(1,k,je+j,i,n);
            if (miux > 0.0)
              ir(k,je+j,i,ang) = ir(k,je,i,n);
            else
              ir(k,je+j,i,ang) = 0.0;
          }
        }
