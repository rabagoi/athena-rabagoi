...
static Real DiskInclination(const Real R);
...
static Real rgap, wgap, gap_sigma;
static Real inc_min, inc_max;
static Real warp_rin, warp_rout;
...
void Mesh::InitUserMeshData(ParameterInput *pin) {
  ...
  // NOTE: For the static warp problem generator, the binary source terms are disabled
  //     and the point source mass is re-enabled.
  ...
  rgap = pin->GetOrAddReal("problem", "rgap", 0.5*(rin_mu+rout_mu));
  wgap = pin->GetOrAddReal("problem", "wgap", 0.0);
  gap_sigma = pin->GetOrAddReal("problem", "gap_sigma", 0.0);

  inc_min = pin->GetOrAddReal("problem", "inc_min", 0.0);
  inc_max = pin->GetOrAddReal("problem", "inc_max", PI/12);

  warp_rin = pin->GetOrAddReal("problem", "warp_rin", 0.8);
  warp_rout = pin->GetOrAddReal("problem", "warp_rout", 1.2);
...
static void GetCylCoord(Coordinates *pco,Real &rad,Real &phi,Real &z,int i,int j,int k) {
  ...
  // Calculate the local inclination from the min and max inclination limits
  //Real inc = DiskInclination(pco->x1v(i));
  Real inc = pco->x1v(i) < rgap ? inc_min : inc_max;
...
//----------------------------------------------------------------------------------------
//!\f: Disk inclination, as a function of spherical radius
//
static Real DiskInclination(const Real R)
{
  Real inc;
  ...
  else if (warp_rin <= R && R <= warp_rout)
  {
    Real inc_offset = (inc_min + inc_max)/2.0;
    Real inc_amp = (inc_max - inc_min)/2.0;
    inc = inc_offset + inc_amp*( -1.0*cos(PI/(warp_rout-warp_rin)*(R-warp_rin)) );
  }
  else
  {
    inc = inc_max;
